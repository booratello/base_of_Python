...
"""
В этом варианте можно складывать больше 2 матриц и присваивать сумму переменной, но основной минус тот же - каждую
матрицу / "зафиксированную" сумму лучше испольльзовать не более раза за вызов программы.
"""
...
class Matrix:
    ...
    def __init__(self, rows, columns, sum_matrix=None):
        self.rows = self.is_it_number(rows)
        self.columns = self.is_it_number(columns)
        if sum_matrix is None:
            self.matrix_double_list = self.matrix_input()
        else:
            self.matrix_double_list = sum_matrix
    ...
    def matrix_input(self):
        while True:
            user_answer = self.is_it_number(input("Будем заполнять матрицу вручную [1], или доверемся рандому [2] "
                                                  "(числа от 0 до 100)?\n"))
            if user_answer == 1:
                return [[self.is_it_number(input(f"Введите целое цисло в качестве элемента матрицы "
                                                 f"[{i + 1}][{j + 1}]\n"))
                         for j in range(self.columns)] for i in range(self.rows)]
            elif user_answer == 2:
                return [[randrange(101) for _1 in range(self.columns)] for _2 in range(self.rows)]
            else:
    ...
    def __add__(self, other):
        if self.rows != other.rows or self.columns != other.columns:
            ...
            return Matrix(self.rows, self.columns, self.matrix_double_list)
